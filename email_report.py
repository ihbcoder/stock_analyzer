...
def send_scan_email_report(
    ...
    *,
    ...
    monthly_action_required: bool = False,
) -> None:
    ...
    body = build_email_body(
        ...
        monthly_action_required=monthly_action_required,
    )
    ...
    subject = build_email_subject(result, monthly_action_required=monthly_action_required)
    _send_email(
...
def build_email_subject(result: dict[str, Any], *, monthly_action_required: bool = False) -> str:
    ...
    if monthly_action_required and run_status == "SUCCESS":
        return f"MONTHLY REBALANCE ACTION REQUIRED | {generated_at}{suffix}"
...
def build_email_body(
    ...
    *,
    ...
    lines: list[str] = [
        "MONTHLY REBALANCE ACTION REQUIRED" if monthly_action_required else "Stock Analyzer Daily Results",
        "",
...
def build_email_html(
    ...
    *,
    ...
    targets = (plan or {}).get("target_allocations", []) or []
    failure_html = (
        f"""
  ...
  <body style="margin:0; padding:0; background:#08101c; color:#e6eef9; font-family:Segoe UI, Arial, sans-serif;">
    <div style="max-width:980px; margin:0 auto; padding:24px 16px;">
      <div style="background:#10192a; border:1px solid #22314c; border-radius:16px; padding:20px; margin-bottom:16px;">
        <div style="font-size:28px; font-weight:700; margin-bottom:8px;">{"MONTHLY REBALANCE ACTION REQUIRED" if monthly_action_required else "Stock Analyzer Daily Results"}</div>
      ...
      {(_render_rebalance_section_html(plan, schedule, summary, sells, buys, targets) if plan is not None else "")}
...
def _render_rebalance_section_html(
    ...
    return f"""
      <div style="background:#10192a; border:1px solid #22314c; border-radius:16px; padding:18px; margin-bottom:16px;">
        ...
        {_render_target_allocations_html(targets)}
      </div>
    """
...
def _render_target_allocations_html(rows: list[dict[str, Any]]) -> str:
    if not rows:
        return ""
    items = "".join(
        f"<li style='margin-bottom:6px;'><strong>{html.escape(str(row.get('ticker') or ''))}</strong> "
        f"- {html.escape(str(row.get('source') or ''))}, "
        f"target={float(row.get('target_weight_pct') or 0):.1f}%</li>"
        for row in rows
    )
    return f"""
      <div style="margin-top:16px;">
        <div style="font-size:15px; font-weight:700; margin-bottom:8px;">Suggested target allocation</div>
        <ul style="margin:0; padding-left:20px; color:#e6eef9; font-size:14px; line-height:1.5;">{items}</ul>
      </div>
    """
