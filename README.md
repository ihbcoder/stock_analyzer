...
- Every successful scheduled scan sends a daily status email. After the close on the first NYSE trading day of a month, that day's email is instead titled `MONTHLY REBALANCE ACTION REQUIRED` and includes the detailed rotation plan. Failure emails are sent immediately on any day.
