...
- A successful scheduled scan sends email only after the close on the first NYSE trading day of a month. That message is titled `MONTHLY REBALANCE ACTION REQUIRED`; daily successful scans still save history but do not email. Failure emails are sent immediately on any day.
